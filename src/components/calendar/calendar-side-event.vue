<template>
  <div ref="eventContainer" :data-date-start="datetime_start" :data-date-end="datetime_end" :data-evendate="eventDate">
    <!-- events header -->
    <div class="w-full day-header text-005743 font-bold text-0dt813 leading-5 mb-2">
    ...
    <div class="w-full">
      ...
      <template v-if="RdvsPkg.length === 0">
       <br>
        <router-link to="/booking" class="bg-booking hover:bg-blue-700 text-white  py-2 px-4 rounded-lg">Booking</router-link> &nbsp;
        <br>
        <br>
      ...
      <template v-else>
        <div
          ...
          v-for="(rdv, rdvi) in RdvsPkg" :key="rdvi" role="button" aria-label="Event" @click="viewEvent(rdv.id)"
          ...
          <div class="w-full flex-shrink more-event-body-item-body">
            <div class="font-semibold text-A1A1AA leading-4 text-0dt688">
              <span :data-event-date="rdv.date" :title="isoStringToDate(rdv.date).toLocaleString($i18n.locale)"
                class="calendar--side-event-time">
                {{ hours(rdv.date) }}:{{ minutes(rdv.date) }}
              </span>
            </div>
            <div class="font-medium text-xs text-09101D flex flex-nowrap items-center">
              <span :title="rdv?.comment ?? ''" class="block capitalize truncate max-w-50p calendar--side-event-name">
                {{ rdv.name }}&nbsp;
              </span>
              <!---->
              <span class="block text-A1A1AA capitalize truncate calendar--side-event-keyword">
                {{ rdv.keywords }}
              </span>
            </div>
          </div>
...
import showdata2 from "../showdata2.vue";
